...
export function* getAllCarts(action: UserId | any): any{
  try {
    ...
    console.log("🚀 ~ file: cart.ts:16 ~ function*getAllCarts ~ userId:", userId)
...
export function* addNewCart({payload: {userId, products, image}}: AddToCart | any): any {
  try {
    ...
    const responseCustom = {...response, products: [{...response.products[0], image}]}
    let data = dataCarts.length
      ? dataCarts.map((cart: any) => {
          if (cart.id === response.id) {
            const product = response.products[0];
            const existingProductIndex = cart.products.findIndex((p: any) => p.id === product.id);
...
export function* updateCart(action: productInfo | any): any{
  try {
    ...
    if(response){
      console.log("🚀 ~ file: cart.ts:71 ~ function*updateCart ~ response:", response)
      // yield put(setDataCarts(response.carts));
    }
  } catch (error) {
